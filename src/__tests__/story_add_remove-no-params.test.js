...
test('should ADD a story to the TOP published and TOP noParams', async () => {
  ...
  expect(countNoParams.innerHTML).toBe(
    (initialNoParamsStoriesCount + 1).toString()
  );
  ...
  expect(noParamsStoriesArr[0]).toBe(addedStory.title);
});
...
test('should REMOVE a story from published && noParams', async () => {
  ...
  expect(noParamsStoriesArr.length).toBe(initialNoParamsStoriesCount - 1);
  expect(noParamsStoriesArr[0]).toBe(stories[1].title);
});
