...
test('should ADD a story to the BOTTOM published with customAdd', async () => {
...
test('should NOT REMOVE a story from published because of customRemove', async () => {
  const { getByText, container, getByTestId } = render(
    <ApolloProvider client={createClient()}>
      <StoriesList customRemove={({ data, query, type, variables }) => data} />
    </ApolloProvider>
  );
  await wait();
